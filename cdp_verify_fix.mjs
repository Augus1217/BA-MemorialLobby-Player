...
// full reload to pick up new app.js
...
async function shot(name) {
  ...
  if (r.result?.data) { writeFileSync(`/tmp/opencode/fix_${name}.png`, Buffer.from(r.result.data, 'base64')); console.log('saved', name); }
}
// screen version (new default)
await shot('screen');
// toggle to additive for comparison
await evalJS('window.ba_debug.setSlotBlend("top_light", 1); window.ba_debug.dbgForceDirty(); true');
await sleep(500);
await shot('add');
// restore screen
