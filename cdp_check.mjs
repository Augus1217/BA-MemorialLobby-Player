...
async function shot(name) {
  ...
  if (r.result?.data) { writeFileSync(`/tmp/opencode/d1_${name}.png`, Buffer.from(r.result.data, 'base64')); console.log('saved', name); }
}
console.log('slot:', JSON.stringify(await evalJS('window.ba_debug.spineSlot("top_light")')));
console.log('live:', JSON.stringify(await evalJS('window.ba_debug.dbgLightLive()')));
await evalJS('window.ba_debug.setTimeScale(0)');
await sleep(500);
await shot('idle_frozen');
// toggle light off/on with proper rebuild
await evalJS('window.ba_debug.dbgForceDirty()');
await sleep(400);
await shot('on');
await evalJS('window.spine.skeleton.findSlot("top_light").setAttachment(null)');
await evalJS('window.ba_debug.dbgForceDirty()');
await sleep(800);
await shot('off');
child.kill();
